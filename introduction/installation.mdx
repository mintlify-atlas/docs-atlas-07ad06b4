...
<Note>
  ...
  if you intend to [build from source](/introduction/building-from-source).
</Note>
...
<Steps>
  ...
  <Step title="Download the archive for your platform">
    ...
    |---|---|
    | `fluXis-win-x64.zip` | Windows 64-bit |
    | `fluXis-osx-x64.tar.gz` | macOS (Intel) |
    | `fluXis-osx-arm64.tar.gz` | macOS (Apple Silicon) |
    | `fluXis-linux-x64.tar.gz` | Linux 64-bit |
  </Step>
  <Step title="Extract the archive">
    Extract the downloaded archive to a folder of your choice. On Windows you
    can right-click and select **Extract All**. On macOS and Linux, use:

    ```bash
    tar -xzf fluXis-linux-x64.tar.gz -C ~/Games/fluXis
    ```
  </Step>
...
After installing fluXis you can immediately populate your library by importing maps from osu!, Quaver, and Stepmania. Go to **Settings → Imports** inside the game and point fluXis at the relevant game's song folder. fluXis will convert and index the maps automatically so they appear in your song select.
